...
export default function KoreanContact() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(""); setSent(false); setSending(true);
    const form = event.currentTarget; const data = new FormData(form);
    const payload = { name: String(data.get("name") || "").trim(), company: String(data.get("company") || "").trim(), email: String(data.get("email") || "").trim(), phone: String(data.get("phone") || "").trim(), inquiry: String(data.get("inquiry") || "").trim(), message: String(data.get("message") || "").trim() };
    if (!payload.name || !payload.email || !payload.inquiry || !payload.message) { setError("필수 항목을 모두 입력해 주세요."); setSending(false); return; }
    try {
      const response = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error || "문의 내용을 보내지 못했습니다.");
      setSent(true); form.reset();
    } catch (err) { setError(err instanceof Error ? err.message : "문의 내용을 보내지 못했습니다. 잠시 후 다시 시도해 주세요."); }
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-2">
          <div className="space-y-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-8"><p className="text-sm font-semibold tracking-[0.2em] text-green-700">전화</p><a href="tel:+622159401278" className="mt-5 flex items-center gap-5 whitespace-nowrap text-xl font-bold tracking-tight text-slate-900 hover:text-green-700 md:text-2xl"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-green-600 text-white" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-7 w-7 fill-current"><path d="M6.6 10.8c1.5 3 3.6 4.9 6.6 6.6l2.2-2.2c.3-.3.8-.4 1.2-.2 1.3.4 2.7.7 3.9.7.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C11.2 21 3 12.8 3 2.5c0-.6.4-1 1-1h3.3c.6 0 1 .4 1 1 0 1.3.2 2.6.7 3.9.1.4 0 .9-.3 1.2l-2.1 2.2Z" /></svg></span><span>+62 21 59401278</span></a></div>
              <div className="rounded-2xl border border-slate-200 p-8"><p className="text-sm font-semibold tracking-[0.2em] text-green-700">이메일</p><a href="mailto:daesoungcs@daesoung.com" className="mt-5 flex items-center gap-5 whitespace-nowrap text-xl font-bold tracking-tight text-slate-900 hover:text-green-700 md:text-2xl"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-green-600 text-white" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-7 w-7 fill-none stroke-current" strokeWidth="2"><path d="M3 5h18v14H3z" /><path d="m3 6 9 7 9-7" /></svg></span><span>daesoungcs@daesoung.com</span></a></div>
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm"><iframe title="대성전자 인도네시아 공장 위치 Google 지도" src={MAP_EMBED} className="h-[520px] w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /><div className="flex items-center justify-between gap-4 border-t border-slate-200 bg-white px-6 py-5"><p className="text-sm text-slate-600">Google 지도에서 회사 위치를 확인하세요.</p><a href={MAP_LINK} target="_blank" rel="noreferrer" className="shrink-0 rounded-full bg-green-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-green-800">Google 지도 열기 ↗</a></div></div>
        </div>
      </div></section>
